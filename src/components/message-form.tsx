...
export function MessageForm() {
  ...
  const [formData, setFormData] = useState<MessageFormValues | null>(null);
  ...
  const handleConfirm = () => {
    if (formData) {
      ...
      startTransition(async () => {
        const response = await generateMessageAction(formData);
        if (response.error) {
          toast({
            variant: 'destructive',
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                ...
                <Button type="submit" disabled={isPending} className="w-full text-white font-bold animated-gradient">
                  ...
                  {hasGeneratedMessage ? 'Generate Another' : 'Compliment Me'}
                </Button>
              </form>
            </Form>
          </CardContent>
